...
public class TrazAqui{
    ...
    private TrazAqui() {
        ...
        this.ent = p.parse();
        try {
            this.ent = GerirTrazAqui.lerObj("estado.obj");
        }
        catch (FileNotFoundException e) {
            System.out.println("Parece que é a primeira utilização");
            this.ent = new GerirTrazAqui();
        }
        catch (IOException e) {
                    ...
                    switch (menuLogin.getOpcao()){
                        case 1:
                            ...
                            do{
                                ...
                                email = lerNome();
                                if (this.ent.existeUtilizador("Utilizador", email)){
                            ...
                            if (!email.equals("0")){
	                            ...
	                            this.ent.addCliente(email, password, nome, gps);
	                        }
                            break;
                        case 2:
                            ...
                            do{
                                System.out.println("Insira o seu email:");
                                email = lerNome();
                                if (this.ent.existeUtilizador("Voluntário", email)){
                                    System.out.println("Esse email já está a ser utilizado");
                                    email = null;
                                }
                            } while(email == null && !email.equals("0"));
                            if (!email.equals("0")){
	                            ...
	                            this.ent.addVoluntario(email, password, nome, gps, raio, meds, vel);
	                        }
                            break;
                        case 3:
                            ...
                            do{
                                ...
                                email = lerNome();
                                if (this.ent.existeUtilizador("Loja", email)){
                                    System.out.println("Esse email já está a ser utilizado");
                                    email = null;
                                }
                            } while(email == null && !email.equals("0"));
                            if (!email.equals("0")){
	                            ...
	                            this.ent.addLoja(email, password, nome, gps, espera);
	                        }
                            break;
                        case 4:
                            ...
                            do{
                                System.out.println("Insira o email:");
                                email = lerNome();
                                if (this.ent.existeUtilizador("Empresa", email)){
                                    System.out.println("Esse email já está a ser utilizado");
                                    email = null;
                                }
                            } while(email == null && !email.equals("0"));
                            if (!email.equals("0")){
	                            ...
	                            this.ent.addEmpresa(email, password, nome, gps, raio, meds, aceite, precokms, precoTemp, vel);
	                        }
                            break;
                    }
                    break;
                    ...
                    switch (menuLogin.getOpcao()){
                        case 1:
                            ...
                            if (!email.equals("0")){
                                ...
                                c = this.ent.getCliente().get(email);
                                while (c.getTransporte().size() > 0){
                                    ...
                                    if (esc == 1){
                                        this.ent.getEmp(c.getTransporte().get(0).getCodBusca()).addEncomenda(c.getTransporte().get(0));
                                        c.getTransporte().remove(0);
                                    }
                                    else{
                                        this.ent.getL(c.getTransporte().get(0).getCodLoja()).getEnc(c.getTransporte().get(0).getCodEnc());
                                        this.ent.getEmp(c.getTransporte().get(0).getCodBusca()).setAceite(this.ent.getEmp(c.getTransporte().get(0).getCodBusca()).getAceite() + 1);
                                        c.getTransporte().remove(0);
                                    }
                                }
                                while (c.getClassificaVoluntarios().size() > 0){
                                    ...
                                    this.ent.classificarVoluntario(c.getClassificaVoluntarios().get(0), classificacao);
                                }
                                do{
                                    menuCliente.executa();
                                    String prod;
                        ...
                        case 2:
                            ...
                            if (!email.equals("0")){
                                ...
                                v = this.ent.getVoluntario().get(email);
                                do{
                                    ...
                                    switch (menuVoluntario.getOpcao()){
                                        case 1:
                                            ...
                                            if (!l.equals("0")){
                                                do{
                                                    ...
                                                    esc = lerInt();
                                                    if (esc < 0 || esc > a || !this.ent.getUtil(l.getEncomenda().get(esc - 1).getCodUtil()).getLocalizacao().inRangeV(v) || !(!l.getEncomenda().get(esc - 1).getMedicamentos() || v.aceitoTransporteMedicamentos())){
                                        ...
                                        case 3:
                                            ...
                                            this.ent.getL(v.getEncomenda().getCodLoja()).removeEncomenda(v.getEncomenda());
                                            this.ent.entregaV(v);
                                            this.ent.getUtil(v.getEncomenda().getCodUtil()).addCVol(v.getCodigoV());
                                            break;
                                        case 4:
                        ...
                        case 4:
                            ...
                            if (!email.equals("0")){
                                ...
                                t = this.ent.getEmpresas().get(email);
                                do{
                                    menuEmpresa.executa();
                                    switch (menuEmpresa.getOpcao()){
                                        case 1:
                                            if (t.getAceite() > 0){
                                                ...
                                                if (!nomeL.equals("0") && l != null){
                                                    do{
                                                        if (l.getEncomenda().size() > 0){
                                                            ...
                                                            esc = lerInt();
                                                            if (esc == 0) break;
                                                            if (esc < 0 || esc > a || !this.ent.getUtil(l.getEncomenda().get(esc - 1).getCodUtil()).getLocalizacao().inRangeT(t) || !(!l.getEncomenda().get(esc - 1).getMedicamentos() || t.aceitoTransporteMedicamentos())){
                                                                System.out.println("Encomenda inválida");
                                                                esc = -1;
                                                            }
                                                        }
                                                ...
                                                if (!nomeL.equals("0") && esc > 0){
                                                    ...
                                                    e.setPreco(Math.round((e.getPreco() + t.calculaPreco(l, this.ent.getUtil(e.getCodUtil()))) * 100) / 100);
                                                    e.setCodBusca(t.getCodigoT());
                                                    this.ent.getUtil(e.getCodUtil()).addTransporte(e);
                                                    l.getEncomenda().get(esc - 1).setDisponivel(false);
                                        ...
                                        case 3:
                                            if (t.getEncAtuais().size() > 0){
                                                ...
                                                if (esc > 0){
                                                    t.setKms(t.getKms() + this.ent.getUtil(t.getEncAtuais().get(esc - 1).getCodUtil()).getLocalizacao().distEntre(t) + this.ent.getL(t.getEncAtuais().get(esc - 1).getCodLoja()).getLocalizacao().distEntre(t));
    ...
    public boolean login(String email, String password, String cod){
        if (!ent.existeUtilizador(cod, email)){
            System.out.println("Esse email não está registado");
            return false;
        }
        if (password.equals(ent.getUtilizador(cod, email).getPassword())){
            return true;
        }
    ...
    public void verLojas(){
        int i = 1;
        for(Lojas l: this.ent.getLojas().values()){
            System.out.print(i);
            System.out.print(" - ");
    ...
    public int escLoja(){
        ...
        if (op < 0 || op > this.ent.getLojas().size()){
            System.out.println("Opção inválida");
            op = -1;
        }
        return op;
        ...
        for (Lojas l: this.ent.getLojas().values()){
            if (l.getLocalizacao().inRangeV(v)){
                ...
                for (Encomendas e: l.getNaoAceite()){
                    if (this.ent.getUtil(e.getCodUtil()).getLocalizacao().inRangeV(v) && (!e.getMedicamentos() || e.getMedicamentos() == v.aceitoTransporteMedicamentos())){
                        System.out.println(e.toString());
                    }
                }
            }
    ...
    public void verEncomendasT(Transportadora t){
        for (Lojas l: this.ent.getLojas().values()){
            if (l.getLocalizacao().inRangeT(t)){
                System.out.println(l.toString());
                for (Encomendas e: l.getNaoAceite()){
                    if (this.ent.getUtil(e.getCodUtil()).getLocalizacao().inRangeT(t) && (!e.getMedicamentos() || e.getMedicamentos() == t.aceitoTransporteMedicamentos())){
                        System.out.println(e.toString());
                    }
                }
            }
    ...
    public void verTopClientes(){
        List<Cliente> l = this.ent.ordenarCliente(this.ent.getComparadores());
    ...
    public void verTopEmpresas(){
        List<Transportadora> l = this.ent.ordenarTransportadora(this.ent.getComparadoresT());
        for (int i = 0; i < 10 && i < l.size(); i++){
            System.out.print(i + 1);
            System.out.print(" - ");
            System.out.println(l.get(i));
